...
def add_user_with_time(user_id, new_user_time, sub, first_name, last_name):
    with connection.cursor() as cur:
        cur.execute("INSERT INTO sessiyabot.users (id,notifytime,subscribe,firstname,lastname) VALUES (%s,%s,%s,%s,%s);", (user_id, new_user_time, sub, first_name, last_name))
        connection.commit()

def add_user_with_date(user_id, new_user_date, first_name, last_name):
    with connection.cursor() as cur:
        cur.execute("INSERT INTO sessiyabot.users (id,examdate,firstname,lastname) VALUES (%s,%s,%s,%s);", (user_id, new_user_date, first_name, last_name))
        connection.commit()

def add_user_with_tz(user_id, new_tz, first_name, last_name):
    with connection.cursor() as cur:
        cur.execute("INSERT INTO sessiyabot.users (id,tz,firstname,lastname) VALUES (%s,%s,%s,%s);", (user_id, new_tz, first_name, last_name))
        connection.commit()

def update_user_time_and_sub(user_id, new_user_time, sub):
    with connection.cursor() as cur:
        cur.execute("UPDATE sessiyabot.users SET notifytime=%s,subscribe=%s WHERE id=%s;", (new_user_time, sub, user_id))
        connection.commit()
